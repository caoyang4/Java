 ...
 *   CGLib 底层是ASM字节码生成框架，其原理是通过字节码技术为一个类创建子类，并在子类中采用方法拦截的技术拦截所有父类方法的调用，顺势织入横切逻辑，来完成动态代理的实现。
 *   实现方式实现 MethodInterceptor 接口，重写 intercept 方法，通过 Enhancer 类的回调方法来实现。
 *   同时，由于CGLib由于是采用动态创建子类的方法，对于final方法，无法进行代理，没有接口也能实现动态代理，而且采用字节码增强技术，性能也不错。
 *   只有在大量调用的时候cglib的效率高，但是在1.8的时候JDK的效率已高于cglib
 *   Cglib动态代理执行代理方法效率之所以比JDK高是因为Cglib采用了FastClass机制，它为代理类和被代理类各生成了一个class，
 *   这个class会为代理类与被代理类的方法分类index
 */
