 ...
 * @since 13.0
 */
public abstract class GuavaRateLimiter extends AbstractRateLimiter {
    /**
     ...
     */
    public static GuavaRateLimiter create(double permitsPerSecond) {
        return create(permitsPerSecond, 1.0);
    }

    public static GuavaRateLimiter create(double permitsPerSecond, double maxBurstSeconds) {
    /*
     ...
     */
    static GuavaRateLimiter create(SleepingStopwatch stopwatch, double permitsPerSecond, double maxBurstSeconds) {
        GuavaRateLimiter rateLimiter = new SmoothRateLimiter.SmoothBursty(stopwatch, maxBurstSeconds /* maxBurstSeconds */);
     ...
     */
    public static GuavaRateLimiter create(int permitsPerSecond, long warmupPeriod, TimeUnit unit) {
    ...
    static GuavaRateLimiter create(
            SleepingStopwatch stopwatch, double permitsPerSecond, long warmupPeriod, TimeUnit unit) {
        GuavaRateLimiter rateLimiter = new SmoothRateLimiter.SmoothWarmingUp(stopwatch, warmupPeriod, unit);
        rateLimiter.setRate(permitsPerSecond);
        return rateLimiter;
    }
    ...
    GuavaRateLimiter(SleepingStopwatch stopwatch) {
    ...
    /**
     * acquire permits, return microsToWait
     * @param permits
     * @return
     */
    ...
    /**
     * Acquires permits from this {@link GuavaRateLimiter} if it can be acquired immediately without delay.
     * <p>
     * <p>
    ...
    /**
     * Acquires a permit from this {@link GuavaRateLimiter} if it can be acquired immediately without
     * delay.
     * <p>
     * <p>
    ...
    /**
     * try to acquire permits, can acquire return microsToWait,else return -1
     * @param permits
     * @param timeout
     * @param unit
     * @return
     */
    public long tryAcquireNoWait(int permits, long timeout, TimeUnit unit) {
        long timeoutMicros = max(unit.toMicros(timeout), 0);
        checkPermits(permits);
        long microsToWait;
        synchronized (mutex()) {
